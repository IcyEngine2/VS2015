...
css_error css__lexer_create(parserutils_inputstream *input, css_lexer **lexer)
{
	...
	lex = input->realloc(0, sizeof(css_lexer), input->user);
	if (lex == NULL)
		return CSS_NOMEM;
...
css_error css__lexer_destroy(css_lexer *lexer)
{
	...
	lexer->input->realloc(lexer, 0, lexer->input->user);
...
css_error consumeEscape(css_lexer *lexer, bool nl)
{
	...
	if (lexer->unescapedTokenData == NULL) {
		perror = parserutils_buffer_create(lexer->realloc, lexer->user, &lexer->unescapedTokenData);
		if (perror != PARSERUTILS_OK)
			return css_error_from_parserutils_error(perror);
	}
