				...
				css_computed_counter *))
{
	...
	if (isInherit(opv) == false) {
		...
		case COUNTER_INCREMENT_NAMED:
		{
			...
			while (v != COUNTER_INCREMENT_NONE) {
				...
				temp = style->sheet->ctx->realloc(counters,
						(n_counters + 1) *
						sizeof(css_computed_counter), style->sheet->ctx->user);
				if (temp == NULL) {
					if (counters != NULL) {
						style->sheet->ctx->realloc(counters, 0, style->sheet->ctx->user);
					}
					return CSS_NOMEM;
				}
	...
	if (n_counters > 0) {
		...
		temp = style->sheet->ctx->realloc(counters, (n_counters + 1) *
				sizeof(css_computed_counter), style->sheet->ctx->user);
		if (temp == NULL) {
			style->sheet->ctx->realloc(counters, 0, style->sheet->ctx->user);
			return CSS_NOMEM;
