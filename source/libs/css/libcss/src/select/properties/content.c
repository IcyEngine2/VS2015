		...
		css_select_state *state)
{
	...
	if (isInherit(opv) == false) {
		...
		if (v == CONTENT_NORMAL) {
			...
			while (v != CONTENT_NORMAL) {
				...
				temp = style->sheet->ctx->realloc(content,
						(n_contents + 1) *
						sizeof(css_computed_content_item), style->sheet->ctx->user);
				if (temp == NULL) {
					if (content != NULL) {
						style->sheet->ctx->realloc(content, 0, style->sheet->ctx->user);
					}
					return CSS_NOMEM;
				}
	...
	if (n_contents > 0) {
		...
		temp = style->sheet->ctx->realloc(content, (n_contents + 1) *
				sizeof(css_computed_content_item), style->sheet->ctx->user);
		if (temp == NULL) {
			style->sheet->ctx->realloc(content, 0, style->sheet->ctx->user);
			return CSS_NOMEM;
		...
		css_computed_style *style)
{
	...
	for (item = hint->data.content; item != NULL &&
		...
		case CSS_COMPUTED_CONTENT_STRING:
			lwc_string_unref(style->ctx, item->data.string);
			break;
		case CSS_COMPUTED_CONTENT_URI:
			lwc_string_unref(style->ctx, item->data.uri);
			break;
		case CSS_COMPUTED_CONTENT_COUNTER:
			lwc_string_unref(style->ctx, item->data.counter.name);
			break;
		case CSS_COMPUTED_CONTENT_COUNTERS:
			lwc_string_unref(style->ctx, item->data.counters.name);
			lwc_string_unref(style->ctx, item->data.counters.sep);
			break;
		case CSS_COMPUTED_CONTENT_ATTR:
			lwc_string_unref(style->ctx, item->data.attr);
			break;
		default:
			break;
		}
	...
	if (error != CSS_OK && hint->data.content != NULL)
		style->ctx->realloc(hint->data.content, 0, style->ctx->user);
		...
		css_computed_style *result)
{
	...
	if (type == CSS_CONTENT_SET) {
		...
		copy = parent->ctx->realloc(0, (n_items + 1) *
				sizeof(css_computed_content_item), parent->ctx->user);
		if (copy == NULL)
			return CSS_NOMEM;
