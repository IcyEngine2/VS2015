		...
		css_select_state *state)
{
	...
	if (isInherit(opv) == false) {
		...
		while (v != QUOTES_NONE) {
			...
			temp = style->sheet->ctx->realloc(quotes,
					(n_quotes + 2) * sizeof(lwc_string *), style->sheet->ctx->user);
			if (temp == NULL) {
				if (quotes != NULL) {
					style->sheet->ctx->realloc(quotes, 0, style->sheet->ctx->user);
				}
				return CSS_NOMEM;
			}
	...
	if (n_quotes > 0) {
		...
		temp = style->sheet->ctx->realloc(quotes, (n_quotes + 1) * sizeof(lwc_string *), style->sheet->ctx->user);
		if (temp == NULL) {
			style->sheet->ctx->realloc(quotes, 0, style->sheet->ctx->user);
			return CSS_NOMEM;
		}
		...
		css_computed_style *result)
{
	...
	if (type == CSS_QUOTES_INHERIT || result != child) {
		...
		if (quotes != NULL) {
			...
			copy = parent->ctx->realloc(0, (n_quotes + 1) * sizeof(lwc_string *), parent->ctx->user);
			if (copy == NULL)
				return CSS_NOMEM;
