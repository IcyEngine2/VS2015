		...
		css_select_state *state)
{
	...
	if (isInherit(opv) == false) {
		...
		while (v != VOICE_FAMILY_END) {
			...
			if (value == 0 && voice != NULL) {
				temp = style->sheet->ctx->realloc(voices,
					(n_voices + 1) * sizeof(lwc_string *), style->sheet->ctx->user);
				if (temp == NULL) {
					if (voices != NULL) {
						style->sheet->ctx->realloc(voices, 0, style->sheet->ctx->user);
					}
					return CSS_NOMEM;
				}
	...
	if (n_voices > 0) {
		...
		temp = style->sheet->ctx->realloc(voices, (n_voices + 1) * sizeof(lwc_string *), style->sheet->ctx->user);
		if (temp == NULL) {
			style->sheet->ctx->realloc(voices, 0, style->sheet->ctx->user);
			return CSS_NOMEM;
		}
