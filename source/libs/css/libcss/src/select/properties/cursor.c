		...
		css_select_state *state)
{
	...
	if (isInherit(opv) == false) {
		...
		while (v == CURSOR_URI) {
			...
			temp = style->sheet->ctx->realloc(uris,
					(n_uris + 1) * sizeof(lwc_string *), style->sheet->ctx->user);
			if (temp == NULL) {
				if (uris != NULL) {
					style->sheet->ctx->realloc(uris, 0, style->sheet->ctx->user);
				}
				return CSS_NOMEM;
			}
		...
		css_computed_style *result)
{
	...
	if (urls != NULL) {
		...
		copy = parent->ctx->realloc(0, (n_urls + 1) *
				sizeof(lwc_string *), parent->ctx->user);
		if (copy == NULL)
			return CSS_NOMEM;
