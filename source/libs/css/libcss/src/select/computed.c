 ...
 */
css_error css__computed_style_create(lwc_context* ctx, css_computed_style **result)
{
	...
	s = ctx->realloc(0, sizeof(css_computed_style), ctx->user);
	if (s == NULL)
		return CSS_NOMEM;
...
css_error css_computed_style_destroy(css_computed_style *style)
{
	...
	if (style->counter_increment != NULL) {
		...
		for (c = style->counter_increment; c->name != NULL; c++) {
			lwc_string_unref(style->ctx, c->name);
		}

		style->ctx->realloc(style->counter_increment, 0, style->ctx->user);
	}
	...
	if (style->counter_reset != NULL) {
		...
		style->ctx->realloc(style->counter_reset, 0, style->ctx->user);
	}
	...
	if (style->cursor != NULL) {
		...
		for (s = style->cursor; *s != NULL; s++) {
			lwc_string_unref(style->ctx, *s);
		}

		style->ctx->realloc(style->cursor, 0, style->ctx->user);
	}
	...
	if (style->content != NULL) {
		...
		for (c = style->content;
			...
			case CSS_COMPUTED_CONTENT_STRING:
				lwc_string_unref(style->ctx, c->data.string);
				break;
			case CSS_COMPUTED_CONTENT_URI:
				lwc_string_unref(style->ctx, c->data.uri);
				break;
			case CSS_COMPUTED_CONTENT_ATTR:
				lwc_string_unref(style->ctx, c->data.attr);
				break;
			case CSS_COMPUTED_CONTENT_COUNTER:
				lwc_string_unref(style->ctx, c->data.counter.name);
				break;
			case CSS_COMPUTED_CONTENT_COUNTERS:
				lwc_string_unref(style->ctx, c->data.counters.name);
				lwc_string_unref(style->ctx, c->data.counters.sep);
				break;
			default:
				break;
			}
		...
		style->ctx->realloc(style->content, 0, style->ctx->user);
	}
	...
	if (style->font_family != NULL) {
		...
		style->ctx->realloc(style->font_family, 0, style->ctx->user);
	}
	...
	if (style->quotes != NULL) {
		...
		style->ctx->realloc(style->quotes, 0, style->ctx->user);
	}

	if (style->i.list_style_image != NULL)
		lwc_string_unref(style->ctx, style->i.list_style_image);

	if (style->i.background_image != NULL)
		lwc_string_unref(style->ctx, style->i.background_image);

	style->ctx->realloc(style, 0, style->ctx->user);
		...
		css_computed_style ** result)
{
	 ...
	 */
	error = css__computed_style_create(ctx, &composed);
	if (error != CSS_OK) {
		return error;
	}
