...
static void font_faces_srcs_destroy(lwc_context* ctx, css_font_face *font_face)
{
	uint32_t i;
	...
	for (i = 0; i < font_face->n_srcs; ++i) {
		if (srcs[i].location != NULL) {
			lwc_string_unref(ctx, srcs[i].location);
		}
	}

	ctx->realloc(srcs, 0, ctx->user);
	font_face->srcs = NULL;
}
...
css_error css__font_face_create(lwc_context* ctx, css_font_face **result)
{
	...
	f = ctx->realloc(0, sizeof(css_font_face), ctx->user);
	if (f == NULL)
		return CSS_NOMEM;
...
css_error css__font_face_destroy(css_font_face *font_face)
{
	...
	if (font_face->font_family != NULL)
		lwc_string_unref(font_face->ctx, font_face->font_family);

	if (font_face->srcs != NULL)
		font_faces_srcs_destroy(font_face->ctx, font_face);

	if (font_face->ranges != NULL)
		font_face->ctx->realloc(font_face->ranges, 0, font_face->ctx->user);

	font_face->ctx->realloc(font_face, 0, font_face->ctx->user);
