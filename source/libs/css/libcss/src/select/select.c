...
static css_error css__create_node_data(lwc_context* ctx, struct css_node_data **node_data)
{
	struct css_node_data *nd;

	nd = ctx->realloc(0, sizeof(struct css_node_data), ctx->user);
	if (nd == NULL) {
		return CSS_NOMEM;
	}
	memset(nd, 0, sizeof(struct css_node_data));
	*node_data = nd;

	return CSS_OK;
}

static void css__destroy_node_data(lwc_context* ctx, struct css_node_data *node_data)
{
	int i;
	...
	if (node_data->bloom != NULL) {
		ctx->realloc(node_data->bloom, 0, ctx->user);
	}
	...
	ctx->realloc(node_data, 0, ctx->user);
}


/* Exported function documented in public select.h header. */
css_error css_libcss_node_data_handler(lwc_context* ctx, css_select_handler *handler,
		...
		void *clone_node, void *libcss_node_data)
{
	...
	case CSS_NODE_DELETED:
		css__destroy_node_data(ctx, node_data);
		break;
 ...
 */
css_error css_select_ctx_create(lwc_context* ctx, css_select_ctx **result)
{
	css_select_ctx *c;
	css_error error;
	...
	c = ctx->realloc(0, sizeof(css_select_ctx), ctx->user);
	if (c == NULL)
		return CSS_NOMEM;
...
css_error css_select_ctx_destroy(css_select_ctx *ctx)
{
	...
	if (ctx->sheets != NULL) {
		for (uint32_t index = 0; index < ctx->n_sheets; index++) {
			css__mq_query_destroy(ctx->ctx, ctx->sheets[index].media);
		}
		ctx->ctx->realloc(ctx->sheets, 0, ctx->ctx->user);
	}

	ctx->ctx->realloc(ctx, 0, ctx->ctx->user);
		...
		css_origin origin, const char *media)
{
	...
	temp = sheet->ctx->realloc(ctx->sheets,
			(ctx->n_sheets + 1) * sizeof(css_select_sheet), sheet->ctx->user);
	if (temp == NULL)
		return CSS_NOMEM;
	...
	error = css_parse_media_query(sheet->ctx, sheet->propstrings,
			(const uint8_t *)media,
	...
	if (error == CSS_NOMEM) {
		...
		mq = sheet->ctx->realloc(0, sizeof(*mq), sheet->ctx->user);
		if (mq == NULL) {
			return CSS_NOMEM;
		}
		...
		css_select_handler *handler, void *pw)
{
	...
	error = css__computed_style_create(ctx->ctx, &style);
	if (error != CSS_OK)
		return error;
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
static css_error css__get_parent_bloom(lwc_context* ctx, void *parent,
		css_select_handler *handler, void *pw,
		css_bloom **parent_bloom)
{
	...
	if (bloom == NULL) {
		...
		if (parent != NULL) {
			 ...
			 */
			bloom = ctx->realloc(0, sizeof(css_bloom) * CSS_BLOOM_SIZE, ctx->user);
			if (bloom == NULL) {
				return CSS_NOMEM;
			}
			...
			if (node_data == NULL) {
				error = css__create_node_data(ctx, &node_data);
				if (error != CSS_OK) {
					ctx->realloc(bloom, 0, ctx->user);
					return error;
				}
...
static css_error css__create_node_bloom(lwc_context* ctx,
		css_bloom **node_bloom, css_select_state *state)
{
	...
	*node_bloom = NULL;
	...
	memset(bloom, sizeof(css_bloom) * CSS_BLOOM_SIZE, 0);

	/* Add node name to bloom */
	if (lwc_string_caseless_hash_value(ctx, state->element.name,
			&hash) != lwc_error_ok) {
		error = CSS_NOMEM;
		goto cleanup;
	}
	...
	if (state->id != NULL) {
		if (lwc_string_caseless_hash_value(ctx, state->id,
				&hash) != lwc_error_ok) {
			error = CSS_NOMEM;
			goto cleanup;
		}
	...
	if (state->classes != NULL) {
		for (uint32_t i = 0; i < state->n_classes; i++) {
			...
			if (lwc_string_caseless_hash_value(ctx, s,
					&hash) != lwc_error_ok) {
				error = CSS_NOMEM;
				goto cleanup;
			}
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
static css_error css__set_node_data(lwc_context* ctx, void *node, css_select_state *state,
		css_select_handler *handler, void *pw)
{
	int i;
	css_error error;
	...
	error = css__create_node_bloom(ctx, &bloom, state);
	if (error != CSS_OK) {
		return error;
	}
	node_data->bloom = bloom;
		...
		struct css_node_data **sharable_node_data)
{
	...
	*sharable_node_data = NULL;
	...
	if (error != CSS_OK) {
		...
		lwc_string_unref(ctx, share_candidate_id);
#ifdef DEBUG_STYLE_SHARING
	...
	for (uint32_t i = 0; i < share_candidate_n_classes; i++) {
		bool match = false;
		if (lwc_string_caseless_isequal(ctx, state->classes[i],
...
cleanup:
	if (share_candidate_classes != NULL) {
		for (uint32_t i = 0; i < share_candidate_n_classes; i++) {
			lwc_string_unref(ctx, share_candidate_classes[i]);
		}
	}
 ...
 * \return CSS_OK on success or appropriate error otherwise.
 */
static css_error css_select_style__get_sharable_node_data(lwc_context* ctx,
...
printf("      \t%s\tno share: inline style\n");
	...
	while (true) {
		 ...
		 * prevent sharing. */
		error = css_select_style__get_sharable_node_data_for_candidate(ctx,
 ...
 */
static void css_select__finalise_selection_state(lwc_context* ctx,
		css_select_state *state)
{
	...
	if (state->node_data != NULL) {
		css__destroy_node_data(ctx, state->node_data);
	}

	if (state->classes != NULL) {
		for (uint32_t i = 0; i < state->n_classes; i++) {
			lwc_string_unref(ctx, state->classes[i]);
		}
	}

	if (state->id != NULL) {
		lwc_string_unref(ctx, state->id);
	}

	if (state->element.ns != NULL) {
		lwc_string_unref(ctx, state->element.ns);
	}

	if (state->element.name != NULL){
		lwc_string_unref(ctx, state->element.name);
	}
}
 ...
 */
static css_error css_select__initialise_selection_state(lwc_context* ctx,
		css_select_state *state,
		void *node,
		void *parent,
		const css_media *media,
		...
		void *pw)
{
	...
	state->results = ctx->realloc(0, sizeof(css_select_results), ctx->user);
	if (state->results == NULL) {
		return CSS_NOMEM;
	}
	memset(state->results, 0, sizeof(css_select_results));

	error = css__create_node_data(ctx, &state->node_data);
	if (error != CSS_OK) {
		goto failed;
	}

	error = css__get_parent_bloom(ctx, parent, handler, pw,
...
failed:
	css_select__finalise_selection_state(ctx, state);
	return error;
}
		...
		css_select_results **result)
{
	...
	if (ctx == NULL || node == NULL || result == NULL || handler == NULL || media == NULL ||
	    handler->handler_version != CSS_SELECT_HANDLER_VERSION_1)
		return CSS_BADPARM;
	...
	error = css_select__initialise_selection_state(ctx->ctx, &state, node, parent, media, handler, pw);
	if (error != CSS_OK)
		return error;
	...
	error = css_select_style__get_sharable_node_data(ctx->ctx, node, &state, &share);
	if (error != CSS_OK) {
		goto cleanup;
	} else if (share != NULL) {
	 ...
	 */
	error = css__computed_style_create(ctx->ctx,
...
complete:
	error = css__set_node_data(ctx->ctx, node, &state, handler, pw);
	if (error != CSS_OK) {
		goto cleanup;
	}
...
cleanup:
	css_select__finalise_selection_state(ctx->ctx, &state);
		...
		css_select_font_faces_results **result)
{
	...
	if (n_font_faces > 0) {
		...
		results = ctx->ctx->realloc(0, sizeof(css_select_font_faces_results), ctx->ctx->user);
		if (results == NULL) {
			error = CSS_NOMEM;
			goto cleanup;
		}

		results->font_faces = ctx->ctx->realloc(0, 
				n_font_faces * sizeof(css_font_face *), ctx->ctx->user);
		if (results->font_faces == NULL) {
			ctx->ctx->realloc(results, 0, ctx->ctx->user);
			error = CSS_NOMEM;
			goto cleanup;
		}
...
cleanup:
	if (state.ua_font_faces.count != 0)
		ctx->ctx->realloc(state.ua_font_faces.font_faces, 0, ctx->ctx->user);

	if (state.user_font_faces.count != 0)
		ctx->ctx->realloc(state.user_font_faces.font_faces, 0, ctx->ctx->user);

	if (state.author_font_faces.count != 0)
		ctx->ctx->realloc(state.author_font_faces.font_faces, 0, ctx->ctx->user);
...
css_error intern_strings(css_select_ctx *ctx)
{
	...
	/* Universal selector */
	error = lwc_intern_string(ctx->ctx, "*", SLEN("*"), &ctx->universal);
	if (error != lwc_error_ok)
		return css_error_from_lwc_error(error);

	/* Pseudo classes */
	error = lwc_intern_string(ctx->ctx,
...
void destroy_strings(css_select_ctx *ctx)
{
	if (ctx->universal != NULL)
		lwc_string_unref(ctx->ctx, ctx->universal);
	if (ctx->first_child != NULL)
		lwc_string_unref(ctx->ctx, ctx->first_child);
	if (ctx->link != NULL)
		lwc_string_unref(ctx->ctx, ctx->link);
	if (ctx->visited != NULL)
		lwc_string_unref(ctx->ctx, ctx->visited);
	if (ctx->hover != NULL)
		lwc_string_unref(ctx->ctx, ctx->hover);
	if (ctx->active != NULL)
		lwc_string_unref(ctx->ctx, ctx->active);
	if (ctx->focus != NULL)
		lwc_string_unref(ctx->ctx, ctx->focus);
	if (ctx->nth_child != NULL)
		lwc_string_unref(ctx->ctx, ctx->nth_child);
	if (ctx->nth_last_child != NULL)
		lwc_string_unref(ctx->ctx, ctx->nth_last_child);
	if (ctx->nth_of_type != NULL)
		lwc_string_unref(ctx->ctx, ctx->nth_of_type);
	if (ctx->nth_last_of_type != NULL)
		lwc_string_unref(ctx->ctx, ctx->nth_last_of_type);
	if (ctx->last_child != NULL)
		lwc_string_unref(ctx->ctx, ctx->last_child);
	if (ctx->first_of_type != NULL)
		lwc_string_unref(ctx->ctx, ctx->first_of_type);
	if (ctx->last_of_type != NULL)
		lwc_string_unref(ctx->ctx, ctx->last_of_type);
	if (ctx->only_child != NULL)
		lwc_string_unref(ctx->ctx, ctx->only_child);
	if (ctx->only_of_type != NULL)
		lwc_string_unref(ctx->ctx, ctx->only_of_type);
	if (ctx->root != NULL)
		lwc_string_unref(ctx->ctx, ctx->root);
	if (ctx->empty != NULL)
		lwc_string_unref(ctx->ctx, ctx->empty);
	if (ctx->target != NULL)
		lwc_string_unref(ctx->ctx, ctx->target);
	if (ctx->lang != NULL)
		lwc_string_unref(ctx->ctx, ctx->lang);
	if (ctx->enabled != NULL)
		lwc_string_unref(ctx->ctx, ctx->enabled);
	if (ctx->disabled != NULL)
		lwc_string_unref(ctx->ctx, ctx->disabled);
	if (ctx->checked != NULL)
		lwc_string_unref(ctx->ctx, ctx->checked);
	if (ctx->first_line != NULL)
		lwc_string_unref(ctx->ctx, ctx->first_line);
	if (ctx->first_letter != NULL)
		lwc_string_unref(ctx->ctx, ctx->first_letter);
	if (ctx->before != NULL)
		lwc_string_unref(ctx->ctx, ctx->before);
	if (ctx->after != NULL)
		lwc_string_unref(ctx->ctx, ctx->after);
}
...
static css_error _select_font_face_from_rule(lwc_context* ctx,
		...
		css_select_font_faces_state *state)
{
	if (mq_rule_good_for_media((const css_rule *) rule, state->media)) {
		...
		if (lwc_string_isequal(
			...
			new_faces = ctx->realloc(faces->font_faces, new_size, ctx->user);
			if (new_faces == NULL) {
				return CSS_NOMEM;
			}
		...
		css_select_font_faces_state *state)
{
	...
	do {
		...
		if (rule != NULL && rule->type == CSS_RULE_IMPORT) {
			...
			error = _select_font_face_from_rule(sheet->ctx,
		...
		const css_stylesheet *sheet, css_select_state *state)
{
	...
	css_selector_hash_iterator node_iterator = 0;
	const css_selector **id_selectors = &empty_selector;
	css_selector_hash_iterator id_iterator = 0;
	const css_selector ***class_selectors = NULL;
	css_selector_hash_iterator class_iterator = 0;
	const css_selector **univ_selectors = &empty_selector;
	css_selector_hash_iterator univ_iterator = 0;
	...
	if (state->classes != NULL && n_classes > 0) {
		...
		class_selectors = ctx->ctx->realloc(0, n_classes * sizeof(css_selector **), ctx->ctx->user);
	...
	while (_selectors_pending(node_selectors, id_selectors,
		...
		case CSS_SELECT_RULE_SRC_ELEMENT:
			error = node_iterator(ctx->ctx, &req, node_selectors,
					&node_selectors);
			break;

		case CSS_SELECT_RULE_SRC_ID:
			error = id_iterator(ctx->ctx, &req, id_selectors,
					&id_selectors);
			break;

		case CSS_SELECT_RULE_SRC_UNIVERSAL:
			error = univ_iterator(ctx->ctx, &req, univ_selectors,
					&univ_selectors);
			break;

		case CSS_SELECT_RULE_SRC_CLASS:
			req.class = state->classes[src.class];
			error = class_iterator(ctx->ctx, &req, class_selectors[src.class],
...
cleanup:
	if (class_selectors != NULL)
		sheet->ctx->realloc(class_selectors, 0, sheet->ctx->user);
		...
		bool inherit)
{
	...
	if (existing->set == 0) {
		...
		if ((css_origin)existing->origin < state->current_origin) {
