...
static css_error _iterate_elements(lwc_context* ctx,
		const struct css_hash_selection_requirments *req,
		const css_selector **current,
		const css_selector ***next);
static css_error _iterate_classes(lwc_context* ctx,
		const struct css_hash_selection_requirments *req,
		const css_selector **current,
		const css_selector ***next);
static css_error _iterate_ids(lwc_context* ctx,
		const struct css_hash_selection_requirments *req,
		const css_selector **current,
		const css_selector ***next);
static css_error _iterate_universal(lwc_context* ctx,
		const struct css_hash_selection_requirments *req,
		const css_selector **current,
		const css_selector ***next);
 ...
 */
static inline bool _chain_good_for_element_name(lwc_context* ctx, const css_selector *selector,
		const css_qname *qname, const lwc_string *uni)
{
	if (selector->data.qname.name != uni) {
		bool match;
		if (lwc_string_caseless_isequal(ctx,
 ...
 */
css_error css__selector_hash_create(lwc_context* ctx, css_selector_hash **hash)
{
	css_selector_hash *h;
	...
	h = ctx->realloc(0, sizeof(css_selector_hash), ctx->user);
	if (h == NULL)
		return CSS_NOMEM;
	memset(h, 0, sizeof(css_selector_hash));

	/* Element hash */
	h->elements.slots = ctx->realloc(0, DEFAULT_SLOTS * sizeof(hash_entry), ctx->user);
	if (h->elements.slots == NULL) {
		ctx->realloc(h, 0, ctx->user);
		return CSS_NOMEM;
	}
	...
	/* Class hash */
	h->classes.slots = ctx->realloc(0, DEFAULT_SLOTS * sizeof(hash_entry), ctx->user);
	if (h->classes.slots == NULL) {
		ctx->realloc(h->elements.slots, 0, ctx->user);
		ctx->realloc(h, 0, ctx->user);
		return CSS_NOMEM;
	}
	...
	/* ID hash */
	h->ids.slots = ctx->realloc(0, DEFAULT_SLOTS * sizeof(hash_entry), ctx->user);
	if (h->ids.slots == NULL) {
		ctx->realloc(h->classes.slots, 0, ctx->user);
		ctx->realloc(h->elements.slots, 0, ctx->user);
		ctx->realloc(h, 0, ctx->user);
		return CSS_NOMEM;
	}
...
css_error css__selector_hash_destroy(css_selector_hash *hash)
{
	...
	for (i = 0; i < hash->elements.n_slots; i++) {
		for (d = hash->elements.slots[i].next; d != NULL; d = e) {
			...
			hash->ctx->realloc(d, 0, hash->ctx->user);
		}
	}
	hash->ctx->realloc(hash->elements.slots, 0, hash->ctx->user);
	...
	hash->ctx->realloc(hash->classes.slots, 0, hash->ctx->user);
	...
	hash->ctx->realloc(hash->ids.slots, 0, hash->ctx->user);
	...
	hash->ctx->realloc(hash, 0, hash->ctx->user);
		...
		const css_selector ***matched)
{
	...
	if (lwc_string_caseless_hash_value(hash->ctx, req->qname.name,
		...
		const css_selector ***matched)
{
	...
	if (lwc_string_caseless_hash_value(hash->ctx, req->class,
			...
			if (n != NULL) {
				...
				if (match && RULE_HAS_BYTECODE(head)) {
					if (css_bloom_in_bloom(
							head->sel_chain_bloom,
							req->node_bloom) &&
					    _chain_good_for_element_name(hash->ctx,
							head->sel,
							&(req->qname),
							req->uni) &&
		...
		const css_selector ***matched)
{
	...
	if (lwc_string_caseless_hash_value(hash->ctx, req->id,
		...
		const css_selector *selector)
{
	if (head->sel == NULL) {
		...
		hash_entry *entry = ctx->ctx->realloc(0, sizeof(hash_entry), ctx->ctx->user);
		if (entry == NULL)
			return CSS_NOMEM;
		...
		const css_selector *selector)
{
	...
	if (prev == NULL) {
		...
		ctx->ctx->realloc(search, 0, ctx->ctx->user);
 ...
 */
css_error _iterate_elements(lwc_context* ctx,
		const struct css_hash_selection_requirments *req,
		const css_selector **current,
		const css_selector ***next)
{
	...
	if (head != NULL && head->sel != NULL) {
		/* Search through chain for first match */
		while (head != NULL) {
			lerror = lwc_string_caseless_isequal(ctx, name,
 ...
 * If nothing further matches, CSS_OK will be returned and **next == NULL
 */
css_error _iterate_classes(lwc_context* ctx,
		const struct css_hash_selection_requirments *req,
		const css_selector **current,
		const css_selector ***next)
{
	...
	if (head != NULL && head->sel != NULL) {
		...
		while (head != NULL) {
			...
			if (name != NULL) {
				lerror = lwc_string_caseless_isequal(ctx,
						ref, name, &match);
				if (lerror != lwc_error_ok)
					return css_error_from_lwc_error(lerror);
				...
				if (match && RULE_HAS_BYTECODE(head)) {
					if (css_bloom_in_bloom(
							head->sel_chain_bloom,
							req->node_bloom) &&
					    _chain_good_for_element_name(ctx,
							head->sel,
							&(req->qname),
							req->uni) &&
					    mq_rule_good_for_media(
 ...
 * If nothing further matches, CSS_OK will be returned and **next == NULL
 */
css_error _iterate_ids(lwc_context* ctx,
		const struct css_hash_selection_requirments *req,
		const css_selector **current,
		const css_selector ***next)
{
