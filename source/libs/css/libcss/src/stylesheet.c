...
css_error css__stylesheet_string_add(css_stylesheet *sheet, lwc_string *string, uint32_t *string_number)
{
	...
	for (new_string_number = 0;
		...
		if (res != lwc_error_ok) {
			lwc_string_unref(sheet->ctx, string);
	...
	if (sheet->string_vector_c >= sheet->string_vector_l) {
		...
		new_vector = sheet->ctx->realloc(sheet->string_vector,
				new_vector_len * sizeof(lwc_string *), sheet->ctx->user);
		...
		css_stylesheet **stylesheet)
{
	...
	if (params == NULL || params->params_version !=
			...
			params->url == NULL || params->resolve == NULL || params->ctx == NULL ||
			stylesheet == NULL)
		return CSS_BADPARM;

	sheet = params->ctx->realloc(0, sizeof(css_stylesheet), params->ctx->user);
	if (sheet == NULL)
		return CSS_NOMEM;
	memset(sheet, 0, sizeof(css_stylesheet));
	sheet->ctx = params->ctx;
	if (!sheet->ctx)
	{
		params->ctx->realloc(sheet, 0, params->ctx->user);
		return CSS_NOMEM;
	}

	error = css__propstrings_get(sheet->ctx, &sheet->propstrings);
	if (error != CSS_OK) 
	{
		lwc_shutdown(sheet->ctx);
		params->ctx->realloc(sheet, 0, params->ctx->user);
		return error;
	}
	...
	if (params->inline_style) {
		error = css__parser_create_for_inline_style(sheet->ctx, params->charset,
				(params->charset != NULL) ?
				CSS_CHARSET_DICTATED : CSS_CHARSET_DEFAULT,
				&sheet->parser);
	} else {
		error = css__parser_create(sheet->ctx, params->charset,
				(params->charset != NULL) ?
				CSS_CHARSET_DICTATED : CSS_CHARSET_DEFAULT,
				&sheet->parser);
	}
	...
	if (error != CSS_OK) {
		css__propstrings_unref(sheet->ctx);
		lwc_shutdown(sheet->ctx);
		params->ctx->realloc(sheet, 0, params->ctx->user);
		return error;
	}
	...
	error = css__selector_hash_create(sheet->ctx, &sheet->selectors);
	if (error != CSS_OK) {
	...
	sheet->url = params->ctx->realloc(0, strlen(params->url) + 1, params->ctx->user);
	if (sheet->url == NULL) {
	...
	memcpy(sheet->url, params->url, strlen(params->url));
	sheet->url[strlen(params->url)] = 0;

	if (params->title != NULL) {
		sheet->title = params->ctx->realloc(0, strlen(params->title) + 1, params->ctx->user);
		if (sheet->title == NULL) {
			params->ctx->realloc(sheet->url, 0, params->ctx->user);
			css__selector_hash_destroy(sheet->selectors);
			css__language_destroy(sheet->parser_frontend);
			css__parser_destroy(sheet->parser);
			css__propstrings_unref(sheet->ctx);
		...
		memcpy(sheet->title, params->title, strlen(params->title));
		sheet->title[strlen(params->title)] = 0;
	}
...
css_error css_stylesheet_destroy(css_stylesheet *sheet)
{
	...
	if (sheet->title != NULL)
		sheet->ctx->realloc(sheet->title, 0, sheet->ctx->user);
	
	sheet->ctx->realloc(sheet->url, 0, sheet->ctx->user);
	...
	for (index = 0; index < sheet->string_vector_c; index++) {
		lwc_string_unref(sheet->ctx, sheet->string_vector[index]);
	}

	if (sheet->string_vector != NULL)
		sheet->ctx->realloc(sheet->string_vector, 0, sheet->ctx->user);
	
	css__propstrings_unref(sheet->ctx);


	lwc_context* ctx = sheet->ctx;
	sheet->ctx->realloc(sheet, 0, sheet->ctx->user);
...
css_error css__stylesheet_style_create(css_stylesheet *sheet, css_style **style)
{
	...
	s = sheet->ctx->realloc(0, sizeof(css_style), sheet->ctx->user);
	if (s == NULL)
		return CSS_NOMEM;

	s->bytecode = sheet->ctx->realloc(0, sizeof(css_code_t) * CSS_STYLE_DEFAULT_SIZE, sheet->ctx->user);

	if (s->bytecode == NULL) {
		sheet->ctx->realloc(s, 0, sheet->ctx->user); /* do not leak */
		return CSS_NOMEM;
	}
...
css_error css__stylesheet_merge_style(css_style *target, css_style *style)
{
	...
	if (newcode_len > target->allocated) {
		...
		newcode_len &= ~(CSS_STYLE_DEFAULT_SIZE - 1);
		newcode = style->sheet->ctx->realloc(target->bytecode,
				newcode_len * sizeof(css_code_t), style->sheet->ctx->user);
...
css_error css__stylesheet_style_append(css_style *style, css_code_t css_code)
{
	...
	if (style->allocated == style->used) {
		...
		newcode = style->sheet->ctx->realloc(style->bytecode,
				sizeof(css_code_t) * newcode_len, style->sheet->ctx->user);
		if (newcode == NULL)
			return CSS_NOMEM;
...
css_error css__stylesheet_style_destroy(css_style *style)
{
	...
	if (sheet->cached_style == NULL) {
		...
		style->sheet->ctx->realloc(sheet->cached_style->bytecode, 0, style->sheet->ctx->user);
		style->sheet->ctx->realloc(sheet->cached_style, 0, style->sheet->ctx->user);
		sheet->cached_style = style;
		style->used = 0;
	} else {
		style->sheet->ctx->realloc(style->bytecode, 0, style->sheet->ctx->user);
		style->sheet->ctx->realloc(style, 0, style->sheet->ctx->user);
	}
		...
		css_qname *qname, css_selector **selector)
{
	...
	sel = sheet->ctx->realloc(0, sizeof(css_selector), sheet->ctx->user);
	if (sel == NULL)
		return CSS_NOMEM;
		...
		css_selector *selector)
{
	...
	for (c = selector->combinator; c != NULL; c = d) {
		...
		for (detail = &c->data; detail;) {
			if (detail->qname.ns != NULL)
				lwc_string_unref(sheet->ctx, detail->qname.ns);
			lwc_string_unref(sheet->ctx, detail->qname.name);

			if (detail->value_type ==
					CSS_SELECTOR_DETAIL_VALUE_STRING &&
					detail->value.string != NULL) {
				lwc_string_unref(sheet->ctx, detail->value.string);
			}
		...
		sheet->ctx->realloc(c, 0, sheet->ctx->user);
	}
	...
	sheet->ctx->realloc(selector, 0, sheet->ctx->user);
		...
		css_selector **parent, const css_selector_detail *detail)
{
	...
	temp = sheet->ctx->realloc((*parent), sizeof(css_selector) +
			(num_details + 1) * sizeof(css_selector_detail), sheet->ctx->user);
	if (temp == NULL)
		return CSS_NOMEM;
		...
		css_rule **rule)
{
	...
	r = sheet->ctx->realloc(0, required, sheet->ctx->user);
	if (r == NULL)
		return CSS_NOMEM;
...
css_error css__stylesheet_rule_destroy(css_stylesheet *sheet, css_rule *rule)
{
	...
	case CSS_RULE_SELECTOR:
	{
		...
		if (s->selectors != NULL)
			sheet->ctx->realloc(s->selectors, 0, sheet->ctx->user);
	...
	case CSS_RULE_CHARSET:
	{
		...
		lwc_string_unref(sheet->ctx, charset->encoding);
	}
		break;
	case CSS_RULE_IMPORT:
	{
		...
		lwc_string_unref(sheet->ctx, import->url);
		if (import->media != NULL) {
			css__mq_query_destroy(sheet->ctx, import->media);
		}
	...
	case CSS_RULE_MEDIA:
	{
		...
		if (media->media != NULL) {
			css__mq_query_destroy(sheet->ctx, media->media);
		}
	...
	/* Destroy rule */
	sheet->ctx->realloc(rule, 0, sheet->ctx->user);
		...
		css_rule *rule, css_selector *selector)
{
	...
	sels = sheet->ctx->realloc(r->selectors,
			(r->base.items + 1) * sizeof(css_selector *), sheet->ctx->user);
	if (sels == NULL)
		return CSS_NOMEM;
