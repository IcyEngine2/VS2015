...
/* Event handlers */
static css_error language_handle_event(lwc_context* ctx, css_parser_event type,
		...
		void **language)
{
	...
	c = sheet->ctx->realloc(0, sizeof(css_language), sheet->ctx->user);
	if (c == NULL)
		return CSS_NOMEM;

	perror = parserutils_stack_create(sheet->ctx->realloc, sheet->ctx->user, sizeof(context_entry),
...
css_error css__language_destroy(css_language *language)
{
	...
	if (language->default_namespace != NULL)
		lwc_string_unref(language->sheet->ctx, language->default_namespace);

	if (language->namespaces != NULL) {
		for (i = 0; i < language->num_namespaces; i++) {
			lwc_string_unref(language->sheet->ctx, language->namespaces[i].prefix);
			lwc_string_unref(language->sheet->ctx, language->namespaces[i].uri);
		}
		language->sheet->ctx->realloc(language->namespaces, 0, language->sheet->ctx->user);
	}

	parserutils_stack_destroy(language->context);

	language->sheet->ctx->realloc(language, 0, language->sheet->ctx->user);
	return CSS_OK;
}
 ...
 */
css_error language_handle_event(lwc_context* ctx, css_parser_event type,
		const parserutils_vector *tokens, void *pw)
{
	(void)ctx;
...
css_error handleStartAtRule(css_language *c, const parserutils_vector *vector)
{
	...
	if (lwc_string_caseless_isequal(c->sheet->ctx, atkeyword->idata, c->strings[CHARSET],
	...
	} else if (lwc_string_caseless_isequal(c->sheet->ctx, atkeyword->idata,
		...
		if (c->state <= IMPORT_PERMITTED) {
			...
			/* Parse media list */
			error = css__mq_parse_media_list(c->sheet->ctx,
					c->strings, vector, &ctx, &media);
			if (error == CSS_NOMEM) {
				return error;
			} else if (media == NULL) {
				/* Fall back to default media: "all". */
				media = c->sheet->ctx->realloc(0, sizeof(*media), c->sheet->ctx->user);
				if (media == NULL) {
					return CSS_NOMEM;
				}
			...
			if (error != CSS_OK) {
				css__mq_query_destroy(c->sheet->ctx, media);
				return error;
			}
			...
			error = css__stylesheet_rule_set_nascent_import(c->sheet,
					rule, url, media);
			if (error != CSS_OK) {
				lwc_string_unref(c->sheet->ctx, url);
				css__stylesheet_rule_destroy(c->sheet, rule);
				css__mq_query_destroy(c->sheet->ctx, media);
				return error;
			}
	...
	} else if (lwc_string_caseless_isequal(c->sheet->ctx, atkeyword->idata, c->strings[MEDIA],
			&match) == lwc_error_ok && match) {
		css_mq_query *media = NULL;
		...
		c->state = HAD_RULE;
	} else if (lwc_string_caseless_isequal(c->sheet->ctx, atkeyword->idata, c->strings[PAGE],
			&match) == lwc_error_ok && match) {
...
css_error addNamespace(css_language *c, lwc_string *prefix, lwc_string *uri)
{
	if (prefix == NULL) {
		...
		if (c->default_namespace != NULL)
			lwc_string_unref(c->sheet->ctx, c->default_namespace);
		...
		if (idx == c->num_namespaces) {
			...
			css_namespace *ns = c->sheet->ctx->realloc(c->namespaces,
					sizeof(css_namespace) *
						(c->num_namespaces + 1), c->sheet->ctx->user);
		...
		if (c->namespaces[idx].uri != NULL)
			lwc_string_unref(c->sheet->ctx, c->namespaces[idx].uri);
		...
		int *ctx, css_selector_detail *specific)
{
	...
	if (qname.name->insensitive == NULL &&
			lwc__intern_caseless_string(c->sheet->ctx, qname.name) != lwc_error_ok)
		return CSS_NOMEM;
		...
		css_selector_detail_value *value)
{
	...
	if (token->type == CSS_TOKEN_IDENT &&
			lwc_string_caseless_isequal(c->sheet->ctx, token->idata,
		...
		int *ctx, bool in_not, css_selector_detail *specific)
{
	...
	for (lut_idx = 0; lut_idx < N_ELEMENTS(pseudo_lut); lut_idx++) {
		if ((lwc_string_caseless_isequal(c->sheet->ctx, qname.name,
	...
	if (token->type == CSS_TOKEN_FUNCTION) {
		...
		if (fun_type == LANG) {
			...
			if (token->type == CSS_TOKEN_IDENT ||
				 ...
				 * for element names */
				if (qname.name->insensitive == NULL &&
						lwc__intern_caseless_string(c->sheet->ctx,
		...
		const parserutils_vector *vector, int *ctx, css_rule *rule)
{
	...
	for (i = FIRST_PROP; i <= LAST_PROP; i++) {
		...
		if (lwc_string_caseless_isequal(c->sheet->ctx, property->idata, c->strings[i],
				&match) == lwc_error_ok && match)
			break;
	}
	if (i == LAST_PROP + 1)
