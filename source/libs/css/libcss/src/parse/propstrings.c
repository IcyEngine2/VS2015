 ...
 */
css_error css__propstrings_get(lwc_context* ctx, lwc_string ***strings)
{
	if (css__propstrings.count > 0) {
		...
		for (i = 0; i < LAST_KNOWN; i++) {
			lerror = lwc_intern_string(ctx, stringmap[i].data,
					stringmap[i].len,
 ...
 */
void css__propstrings_unref(lwc_context* ctx)
{
	css__propstrings.count--;
	...
	if (css__propstrings.count == 0) {
		...
		for (i = 0; i < LAST_KNOWN; i++)
			lwc_string_unref(ctx, css__propstrings.strings[i]);
	}
}
