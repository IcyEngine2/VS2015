		...
		const css_token *ident)
{
	...
	return (lwc_string_caseless_isequal(c->sheet->ctx, ident->idata, c->strings[SERIF],
			&match) == lwc_error_ok && match) ||
		(lwc_string_caseless_isequal(c->sheet->ctx, ident->idata,
			c->strings[SANS_SERIF], &match) == lwc_error_ok &&
			match) ||
		(lwc_string_caseless_isequal(c->sheet->ctx, ident->idata, c->strings[CURSIVE],
			&match) == lwc_error_ok && match) ||
		(lwc_string_caseless_isequal(c->sheet->ctx, ident->idata, c->strings[FANTASY],
			&match) == lwc_error_ok && match) ||
		(lwc_string_caseless_isequal(c->sheet->ctx, ident->idata,
			c->strings[MONOSPACE], &match) == lwc_error_ok &&
			match) ||
		(lwc_string_caseless_isequal(c->sheet->ctx, ident->idata, c->strings[INHERIT],
			&match) == lwc_error_ok && match) ||
		(lwc_string_caseless_isequal(c->sheet->ctx, ident->idata, c->strings[INITIAL],
			&match) == lwc_error_ok && match) ||
		(lwc_string_caseless_isequal(c->sheet->ctx, ident->idata, c->strings[DEFAULT],
			&match) == lwc_error_ok && match);
}
		...
		css_font_face *font_face)
{
	...
	lwc_string_unref(c->sheet->ctx, string);
		...
		*location_type = CSS_FONT_FACE_LOCATION_TYPE_URI;
		...
		if (token != NULL && token->type == CSS_TOKEN_FUNCTION &&
			...
			error = font_face_src_parse_format(c, vector, ctx,
					format);
			if (error != CSS_OK) {
				lwc_string_unref(c->sheet->ctx, *location);
				return error;
			}
		}
	     	...
	     	css_font_face *font_face)
{
	...
	do {
		 ...
		 */
		new_srcs = c->sheet->ctx->realloc(srcs,
				(n_srcs + 1) * sizeof(css_font_face_src), c->sheet->ctx->user);
		if (new_srcs == NULL) {
			error = CSS_NOMEM;
			goto cleanup;
		}
...
cleanup:
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		if (srcs != NULL)
			c->sheet->ctx->realloc(srcs, 0, c->sheet->ctx->user);
	}
		...
		css_font_face *font_face)
{
	...
	if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata,
			c->strings[NORMAL], &match) == lwc_error_ok && match)) {
		style = CSS_FONT_STYLE_NORMAL;
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata,
		...
		css_font_face *font_face)
{
	...
	do {
		...
		new_ranges = c->sheet->ctx->realloc(ranges,
				(n_ranges + 1) * sizeof(css_font_face_src), c->sheet->ctx->user);
		if (new_ranges == NULL) {
			error = CSS_NOMEM;
			goto cleanup;
		}
...
cleanup:
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		if (ranges != NULL)
			c->sheet->ctx->realloc(ranges, 0, c->sheet->ctx->user);
	}
		...
		int *ctx, css_rule_font_face *rule)
{
	...
	if (font_face == NULL) {
		error = css__font_face_create(c->sheet->ctx, &font_face);
		if (error != CSS_OK) {
			return error;
		}
	...
	if (lwc_string_caseless_isequal(c->sheet->ctx, descriptor->idata,
			c->strings[FONT_FAMILY], &match) == lwc_error_ok &&
			match) {
		return font_face_parse_font_family(c, vector, ctx, font_face);
	} else if (lwc_string_caseless_isequal(c->sheet->ctx, descriptor->idata,
