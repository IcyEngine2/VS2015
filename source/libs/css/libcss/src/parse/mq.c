...
static void css__mq_value_destroy(lwc_context* ctx, css_mq_value *value)
{
	assert(value != NULL);

	if (value->type == CSS_MQ_VALUE_TYPE_IDENT) {
		lwc_string_unref(ctx, value->data.ident);
	}
}

static void css__mq_feature_destroy(lwc_context* ctx, css_mq_feature *feature)
{
	if (feature != NULL) {
		lwc_string_unref(ctx, feature->name);
		css__mq_value_destroy(ctx, &feature->value);
		css__mq_value_destroy(ctx, &feature->value2);
		ctx->realloc(feature, 0, ctx->user);
	}
}

static void css__mq_cond_or_feature_destroy(lwc_context* ctx, css_mq_cond_or_feature *cond_or_feature);

static void css__mq_cond_destroy(lwc_context* ctx, css_mq_cond *cond)
{
	if (cond != NULL) {
		for (uint32_t i = 0; i < cond->nparts; i++) {
			css__mq_cond_or_feature_destroy(ctx, cond->parts[i]);
		}
		ctx->realloc(cond->parts, 0, ctx->user);
		ctx->realloc(cond, 0, ctx->user);
	}
}

static void css__mq_cond_or_feature_destroy(lwc_context* ctx,
		css_mq_cond_or_feature *cond_or_feature)
{
	if (cond_or_feature != NULL) {
		switch (cond_or_feature->type) {
		case CSS_MQ_FEATURE:
			css__mq_feature_destroy(ctx, cond_or_feature->data.feat);
			break;
		case CSS_MQ_COND:
			css__mq_cond_destroy(ctx, cond_or_feature->data.cond);
			break;
		}
		ctx->realloc(cond_or_feature, 0, ctx->user);
	}
}

void css__mq_query_destroy(lwc_context* ctx, css_mq_query *media)
{
	while (media != NULL) {
		css_mq_query *next = media->next;

		css__mq_cond_destroy(ctx, media->cond);
		ctx->realloc(media, 0, ctx->user);

		media = next;
	}
}

static css_error mq_parse_condition(lwc_context* context, lwc_string **strings,
...
static css_error mq_create_feature(lwc_context* ctx,
		lwc_string *name,
		css_mq_feature **feature)
{
	css_mq_feature *f;

	f = ctx->realloc(0, sizeof(*f), ctx->user);
	if (f == NULL) {
		return CSS_NOMEM;
	}
	...
	if (lwc_string_tolower(ctx, name, &f->name) != lwc_error_ok) {
		ctx->realloc(f, 0, ctx->user);
		return CSS_NOMEM;
	}
		...
		css_mq_feature_op *op)
{
	...
	if (len == 2) {
		if (_strnicmp(data, "<=", 2) == 0)
			*op = CSS_MQ_FEATURE_OP_LTE;
		else if (_strnicmp(data, ">=", 2) == 0)
 ...
 * the operator.
 */
static css_error mq_parse_range__convert_to_level_4(lwc_context* ctx,
		css_mq_feature *feature)
{
	lwc_string *new_name;
	...
	if (name[0] == 'm' && name[3] == '-') {
		if (name[1] == 'i' && name[2] == 'n') {
			if (lwc_intern_substring(ctx, feature->name,
...
static css_error mq_parse_range(lwc_context* context, lwc_string **strings,
		const parserutils_vector *vector, int *ctx,
		...
		css_mq_feature **feature)
{
	...
	css_mq_feature_op op = CSS_MQ_FEATURE_OP_BOOL, op2 = CSS_MQ_FEATURE_OP_BOOL;
	css_fixed ratio = 0, ratio2 = 0;
	...
	if (name_or_value->type == CSS_TOKEN_NUMBER &&
		...
		value_is_ratio = true;
			...
			lwc_string_caseless_isequal(context, name_or_value->idata,
	...
	error = mq_create_feature(context, name->idata, &result);
	if (error != CSS_OK) {
		return error;
	}
	if (name_first) {
	...
	if (value_is_ratio) {
		...
		if (error != CSS_OK) {
			css__mq_feature_destroy(context, result);
			return error;
		}
	}
	if (value2 != NULL) {
...
static css_error mq_parse_media_feature(lwc_context* context, lwc_string **strings,
		const parserutils_vector *vector, int *ctx,
		css_mq_feature **feature)
{
	...
	if (name_or_value->type == CSS_TOKEN_IDENT) {
		...
		if (tokenIsChar(token, ')')) {
			/* mf-boolean */
			error = mq_create_feature(context, name_or_value->idata, &result);
			if (error != CSS_OK) {
				return error;
			}
			...
			error = mq_parse_range__convert_to_level_4(context, result);
			if (error != CSS_OK) {
				css__mq_feature_destroy(context, result);
				return error;
			}
			...
			/* mf-range */
			error = mq_parse_range(context, strings, vector, ctx,
...
static css_error mq_parse_media_in_parens(lwc_context* context, lwc_string **strings,
		const parserutils_vector *vector, int *ctx,
		css_mq_cond_or_feature **cond_or_feature)
{
	const css_token *token;
	...
	if (tokenIsChar(token, '(') || (token->type == CSS_TOKEN_IDENT &&
			lwc_string_caseless_isequal(context, token->idata,
				strings[NOT], &match) == lwc_error_ok &&
			match)) {
		css_mq_cond *cond;
		error = mq_parse_condition(context, strings, vector, ctx, true, &cond);
		if (error == CSS_OK) {
			token = parserutils_vector_iterate(vector, ctx);
			if (tokenIsChar(token, ')') == false) {
				return CSS_INVALID;
			...
			result = context->realloc(0, sizeof(*result), context->user);
			if (result == NULL) {
				css__mq_cond_destroy(context, cond);
				return CSS_NOMEM;
			}
			...
			*cond_or_feature = result;
			...
			token->type == CSS_TOKEN_DIMENSION) {
		css_mq_feature *feature;
		error = mq_parse_media_feature(context, strings, vector, ctx, &feature);
		if (error == CSS_OK) {
			result = context->realloc(0, sizeof(*result), context->user);
			if (result == NULL) {
				css__mq_feature_destroy(context, feature);
				return CSS_NOMEM;
			}
			memset(result, 0, sizeof(*result));
		...
		bool permit_or, css_mq_cond **cond)
{
	...
	if (tokenIsChar(token, '(') == false) {
		...
		error = mq_parse_media_in_parens(context, strings,
				vector, ctx, &cond_or_feature);
		if (error != CSS_OK) {
			css__mq_cond_destroy(context, result);
			return CSS_INVALID;
		}

		result->negate = 1;
		result->nparts = 1;
		result->parts = context->realloc(0, sizeof(*result->parts), context->user);
		if (result->parts == NULL) {
			css__mq_cond_or_feature_destroy(context, cond_or_feature);
			css__mq_cond_destroy(context, result);
			return CSS_NOMEM;
		}
	...
	while (token != NULL && tokenIsChar(token, ')') == false &&
			tokenIsChar(token, ',') == false) {
		error = mq_parse_media_in_parens(context, strings, vector, ctx,
				&cond_or_feature);
		if (error != CSS_OK) {
			css__mq_cond_destroy(context, result);
			return CSS_INVALID;
		...
		parts = context->realloc(result->parts,
				(result->nparts+1)*sizeof(*result->parts), context->user);
		if (parts == NULL) {
			css__mq_cond_or_feature_destroy(context, cond_or_feature);
			css__mq_cond_destroy(context, result);
			return CSS_NOMEM;
		...
		if (token != NULL && tokenIsChar(token, ')') == false &&
			...
			if (token->type != CSS_TOKEN_IDENT) {
				css__mq_cond_destroy(context, result);
				return CSS_INVALID;
			} else if (lwc_string_caseless_isequal(context, token->idata,
 ...
 */
static uint64_t mq_parse_type(lwc_context* ctx, lwc_string** strings, lwc_string* type)
{
	bool match;

	if (type == NULL) {
		return CSS_MEDIA_ALL;
	} else if (lwc_string_caseless_isequal(ctx,
			type, strings[AURAL],
...
static css_error mq_parse_media_query(lwc_context* context, lwc_string **strings,
		const parserutils_vector *vector, int *ctx,
		css_mq_query **query)
{
	const css_token *token;
	...
	if (is_condition) {
		/* media-condition */
		error = mq_parse_condition(context, strings, vector, ctx, true,
				&result->cond);
		if (error != CSS_OK) {
			context->realloc(result, 0, context->user);
			return error;
		}
	...
	if (lwc_string_caseless_isequal(context, token->idata,
	...
	result->type = mq_parse_type(context, strings, token->idata);
	...
	if (token != NULL) {
		...
		error = mq_parse_condition(context, strings, vector, ctx, false,
				&result->cond);
		if (error != CSS_OK) {
			context->realloc(result, 0, context->user);
			return error;
 ...
 */
static css_error css__mq_parse__create_not_all(lwc_context* ctx, css_mq_query **not_all_out)
{
	css_mq_query *not_all;

	not_all = ctx->realloc(0, sizeof(*not_all), ctx->user);
	if (not_all == NULL) {
		return CSS_NOMEM;
	}
...
css_error css__mq_parse_media_list(lwc_context* context, lwc_string **strings,
		const parserutils_vector *vector, int *ctx,
		css_mq_query **media)
{
	...
	while (token != NULL) {
		...
		error = mq_parse_media_query(context, strings, vector, ctx, &query);
		if (error == CSS_INVALID) {
			error = css__mq_parse__create_not_all(context, &query);
		}

		if (error != CSS_OK) {
			css__mq_query_destroy(context, result);
			return error;
		}
...
static css_error css_parse_media_query_handle_event(lwc_context* context,
		...
		void *pw)
{
	...
	err = css__mq_parse_media_list(context, strings, tokens, &idx, &media);
	if (err != CSS_OK) {
		return CSS_OK;
	}
...
css_error css_parse_media_query(lwc_context* context, lwc_string **strings,
		...
		css_mq_query **media_out)
{
	...
	err = css__parser_create_for_media_query(context, NULL,
