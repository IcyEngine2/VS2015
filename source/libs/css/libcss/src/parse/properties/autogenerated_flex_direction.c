		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[ROW], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_FLEX_DIRECTION, 0,FLEX_DIRECTION_ROW);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[ROW_REVERSE], &match) == lwc_error_ok && match)) {
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[COLUMN_REVERSE], &match) == lwc_error_ok && match)) {
