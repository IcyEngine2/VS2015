		...
		css_style *result)
{
	...
	do {
		...
		if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[THIN], &match) == lwc_error_ok && match)) {
			side_val[side_count] =  BORDER_WIDTH_THIN;
			parserutils_vector_iterate(vector, ctx);
			error = CSS_OK;
		} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[MEDIUM], &match) == lwc_error_ok && match)) {
			side_val[side_count] =  BORDER_WIDTH_MEDIUM;
			parserutils_vector_iterate(vector, ctx);
			error = CSS_OK;
		} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[THICK], &match) == lwc_error_ok && match)) {
			parserutils_vector_iterate(vector, ctx);
			error = CSS_OK;
