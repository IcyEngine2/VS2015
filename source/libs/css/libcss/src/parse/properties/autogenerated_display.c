		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[INLINE], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_INLINE);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[BLOCK], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_BLOCK);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[LIST_ITEM], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_LIST_ITEM);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[RUN_IN], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_RUN_IN);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[INLINE_BLOCK], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_INLINE_BLOCK);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[INLINE_TABLE], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_INLINE_TABLE);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_ROW_GROUP], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE_ROW_GROUP);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_HEADER_GROUP], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE_HEADER_GROUP);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_FOOTER_GROUP], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE_FOOTER_GROUP);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_ROW], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE_ROW);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_COLUMN_GROUP], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE_COLUMN_GROUP);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_COLUMN], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE_COLUMN);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_CELL], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_TABLE_CELL);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[TABLE_CAPTION], &match) == lwc_error_ok && match)) {
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[FLEX], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DISPLAY, 0,DISPLAY_FLEX);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[INLINE_FLEX], &match) == lwc_error_ok && match)) {
