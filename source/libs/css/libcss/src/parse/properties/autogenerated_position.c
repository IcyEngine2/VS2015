		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[LIBCSS_STATIC], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_POSITION, 0,POSITION_STATIC);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[RELATIVE], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_POSITION, 0,POSITION_RELATIVE);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[ABSOLUTE], &match) == lwc_error_ok && match)) {
