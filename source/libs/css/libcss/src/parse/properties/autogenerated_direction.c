		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[LTR], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_DIRECTION, 0,DIRECTION_LTR);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[RTL], &match) == lwc_error_ok && match)) {
