		...
		uint16_t *value)
{
	 ...
	 *	  none)
	 */
	if ((lwc_string_caseless_isequal(c->sheet->ctx,
			ident->idata, c->strings[DISC],
			&match) == lwc_error_ok && match)) {
		*value = LIST_STYLE_TYPE_DISC;
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx,
		...
		uint32_t *result)
{
	...
	for (i = FIRST_COLOUR; i <= LAST_COLOUR; i++) {
		if (lwc_string_caseless_isequal(c->sheet->ctx, data, c->strings[i],
				&match) == lwc_error_ok && match)
			break;
	}
...
{
	if (len == 4) {
		if (_strnicmp(ptr, "grad", 4) == 0)
			*unit = UNIT_GRAD;
		else if (_strnicmp(ptr, "turn", 4) == 0)
			*unit = UNIT_TURN;
		else if (_strnicmp(ptr, "dppx", 4) == 0)
			*unit = UNIT_DPPX;
		else if (_strnicmp(ptr, "dpcm", 4) == 0)
			*unit = UNIT_DPCM;
		else if (_strnicmp(ptr, "vmin", 4) == 0)
			*unit = UNIT_VMIN;
		else if (_strnicmp(ptr, "vmax", 4) == 0)
			*unit = UNIT_VMAX;
		else
			return CSS_INVALID;
	} else if (len == 3) {
		if (_strnicmp(ptr, "kHz", 3) == 0)
			*unit = UNIT_KHZ;
		else if (_strnicmp(ptr, "deg", 3) == 0)
			*unit = UNIT_DEG;
		else if (_strnicmp(ptr, "rad", 3) == 0)
			*unit = UNIT_RAD;
		else if (_strnicmp(ptr, "cap", 3) == 0)
			*unit = UNIT_CAP;
		else if (_strnicmp(ptr, "rem", 3) == 0)
			*unit = UNIT_REM;
		else if (_strnicmp(ptr, "rlh", 3) == 0)
			*unit = UNIT_RLH;
		else if (_strnicmp(ptr, "dpi", 3) == 0)
			*unit = UNIT_DPI;
		else
			return CSS_INVALID;
	} else if (len == 2) {
		if (_strnicmp(ptr, "Hz", 2) == 0)
			*unit = UNIT_HZ;
		else if (_strnicmp(ptr, "ms", 2) == 0)
			*unit = UNIT_MS;
		else if (_strnicmp(ptr, "px", 2) == 0)
			*unit = UNIT_PX;
		else if (_strnicmp(ptr, "ex", 2) == 0)
			*unit = UNIT_EX;
		else if (_strnicmp(ptr, "em", 2) == 0)
			*unit = UNIT_EM;
		else if (_strnicmp(ptr, "in", 2) == 0)
			*unit = UNIT_IN;
		else if (_strnicmp(ptr, "cm", 2) == 0)
			*unit = UNIT_CM;
		else if (_strnicmp(ptr, "mm", 2) == 0)
			*unit = UNIT_MM;
		else if (_strnicmp(ptr, "pt", 2) == 0)
			*unit = UNIT_PT;
		else if (_strnicmp(ptr, "pc", 2) == 0)
			*unit = UNIT_PC;
		else if (_strnicmp(ptr, "ch", 2) == 0)
			*unit = UNIT_CH;
		else if (_strnicmp(ptr, "ic", 2) == 0)
			*unit = UNIT_IC;
		else if (_strnicmp(ptr, "lh", 2) == 0)
			*unit = UNIT_LH;
		else if (_strnicmp(ptr, "vh", 2) == 0)
			*unit = UNIT_VH;
		else if (_strnicmp(ptr, "vw", 2) == 0)
			*unit = UNIT_VW;
		else if (_strnicmp(ptr, "vi", 2) == 0)
			*unit = UNIT_VI;
		else if (_strnicmp(ptr, "vb", 2) == 0)
			*unit = UNIT_VB;
		else
			return CSS_INVALID;
	} else if (len == 1) {
		if (_strnicmp(ptr, "s", 1) == 0)
			*unit = UNIT_S;
		else if (_strnicmp(ptr, "q", 1) == 0)
			*unit = UNIT_Q;
		else
			return CSS_INVALID;
	} else
		...
		lwc_string **result)
{
	...
	perror = parserutils_buffer_create(c->sheet->ctx->realloc, c->sheet->ctx->user, &buffer);
	...
	lerror = lwc_intern_string(c->sheet->ctx, (char *) buffer->data, buffer->length, &interned);
