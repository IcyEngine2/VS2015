		...
		css_style *result)
{
	...
	do {
		...
		if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[NONE], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_NONE;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[HIDDEN], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_HIDDEN;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[DOTTED], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_DOTTED;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[DASHED], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_DASHED;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[SOLID], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_SOLID;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[LIBCSS_DOUBLE], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_DOUBLE;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[GROOVE], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_GROOVE;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[RIDGE], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_RIDGE;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[INSET], &match) == lwc_error_ok && match)) {
			side_val[side_count] = BORDER_STYLE_INSET;
		} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[OUTSET], &match) == lwc_error_ok && match)) {
