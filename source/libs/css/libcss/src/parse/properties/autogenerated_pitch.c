		...
		css_style *result)
{
	...
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[X_LOW], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_PITCH, 0,PITCH_X_LOW);
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[LOW], &match) == lwc_error_ok && match)) {
	...
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[HIGH], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_PITCH, 0,PITCH_HIGH);
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[X_HIGH], &match) == lwc_error_ok && match)) {
