		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[DIGITS], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_SPEAK_NUMERAL, 0,SPEAK_NUMERAL_DIGITS);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[CONTINUOUS], &match) == lwc_error_ok && match)) {
