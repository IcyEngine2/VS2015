		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[PRE], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_WHITE_SPACE, 0,WHITE_SPACE_PRE);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[NOWRAP], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_WHITE_SPACE, 0,WHITE_SPACE_NOWRAP);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[PRE_WRAP], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_WHITE_SPACE, 0,WHITE_SPACE_PRE_WRAP);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[PRE_LINE], &match) == lwc_error_ok && match)) {
