		...
		css_style *result)
{
	...
	if (token->type == CSS_TOKEN_IDENT &&
		(lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[INHERIT],
			&match) == lwc_error_ok && match)) {
		parserutils_vector_iterate(vector, ctx);
		flags = FLAG_INHERIT;
	} else if (token->type == CSS_TOKEN_IDENT &&
		(lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[LEFTWARDS],
			&match) == lwc_error_ok && match)) {
		parserutils_vector_iterate(vector, ctx);
		value = AZIMUTH_LEFTWARDS;
	} else if (token->type == CSS_TOKEN_IDENT &&
		(lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[RIGHTWARDS],
			&match) == lwc_error_ok && match)) {
		parserutils_vector_iterate(vector, ctx);
