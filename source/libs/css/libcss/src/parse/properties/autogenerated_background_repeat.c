		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[NO_REPEAT], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_BACKGROUND_REPEAT, 0,BACKGROUND_REPEAT_NO_REPEAT);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[REPEAT_X], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_BACKGROUND_REPEAT, 0,BACKGROUND_REPEAT_REPEAT_X);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[REPEAT_Y], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_BACKGROUND_REPEAT, 0,BACKGROUND_REPEAT_REPEAT_Y);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[REPEAT], &match) == lwc_error_ok && match)) {
