		...
		css_style *result)
{
	...
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[SHOW], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_EMPTY_CELLS, 0,EMPTY_CELLS_SHOW);
	} else if ((lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[HIDE], &match) == lwc_error_ok && match)) {
