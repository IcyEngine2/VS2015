		...
		css_style *result)
{
	...
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[AUTO], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_FLEX_BASIS, 0,FLEX_BASIS_AUTO);
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(c->sheet->ctx, token->idata, c->strings[CONTENT], &match) == lwc_error_ok && match)) {
