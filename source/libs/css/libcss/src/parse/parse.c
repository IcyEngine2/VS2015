...
static css_error css__parser_create_internal(lwc_context* ctx, const char *charset,
 ...
 */
css_error css__parser_create(lwc_context* ctx, const char *charset, css_charset_source cs_source,
		css_parser **parser)
{
	parser_state initial = { sStart, 0 };

	return css__parser_create_internal(ctx, charset, cs_source,
			initial, parser);
}
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error css__parser_create_for_inline_style(lwc_context* ctx, const char *charset,
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error css__parser_create_for_media_query(lwc_context* ctx, const char *charset,
		css_charset_source cs_source, css_parser **parser)
{
...
css_error css__parser_destroy(css_parser *parser)
{
	...
	parser->ctx->realloc(parser, 0, parser->ctx->user);
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error css__parser_create_internal(lwc_context* ctx, const char *charset,
		css_charset_source cs_source, parser_state initial,
		css_parser **parser)
{
	css_parser *p;
	...
	if (parser == NULL || realloc == NULL)
		return CSS_BADPARM;

	p = ctx->realloc(0, sizeof(css_parser), ctx->user);
	if (p == NULL)
		return CSS_NOMEM;

	perror = parserutils_inputstream_create(ctx->realloc, ctx->user, charset, cs_source,
			css__charset_extract, &p->stream);
	if (perror != PARSERUTILS_OK) {
		ctx->realloc(p, 0, ctx->user);
		return css_error_from_parserutils_error(perror);
	}
	...
	perror = parserutils_stack_create(ctx->realloc, ctx->user, sizeof(parser_state),
	...
	perror = parserutils_vector_create(ctx->realloc, ctx->user, sizeof(css_token),
	...
	perror = parserutils_stack_create(ctx->realloc, ctx->user, sizeof(char),
		...
		*token = parser->pushback;
		...
		if (t->type < CSS_TOKEN_LAST_INTERN && t->data.data != NULL) {
			/* Insert token text into the dictionary */
                        lerror = lwc_intern_string(parser->ctx, (char *)t->data.data,
                                                    t->data.len, &t->idata);
...
css_error parseStart(css_parser *parser)
{
	...
	case Initial:
		...
		if (parser->event != NULL) {
			parser->event(parser->ctx, CSS_PARSER_START_STYLESHEET, NULL,
					parser->event_pw);
		}
...
#endif
	if (parser->event != NULL) {
		parser->event(parser->ctx, CSS_PARSER_END_STYLESHEET, NULL,
				parser->event_pw);
	}
...
#endif
			if (parser->event != NULL) {
				if (parser->event(parser->ctx, CSS_PARSER_START_RULESET,
...
#endif
	if (parser->event != NULL) {
		parser->event(parser->ctx, CSS_PARSER_END_RULESET, NULL, parser->event_pw);
	}
...
#endif
		if (parser->event != NULL) {
			if (parser->event(parser->ctx, CSS_PARSER_START_ATRULE,
					parser->tokens, parser->event_pw) ==
					CSS_INVALID) {
				parser_state to = { sMalformedAtRule, Initial };
...
#endif
	if (parser->event != NULL) {
		parser->event(parser->ctx, CSS_PARSER_END_ATRULE, NULL, parser->event_pw);
	}
...
#endif
		if (parser->event != NULL) {
			parser->event(parser->ctx, CSS_PARSER_START_BLOCK, NULL,
					parser->event_pw);
		}
...
#endif
	if (parser->event != NULL) {
		parser->event(parser->ctx, CSS_PARSER_END_BLOCK, NULL, parser->event_pw);
	}
...
#endif
				if (parser->event != NULL) {
					parser->event(parser->ctx, CSS_PARSER_BLOCK_CONTENT,
							parser->tokens,
							parser->event_pw);
				}
...
#endif
		if (parser->event != NULL) {
			parser->event(parser->ctx, CSS_PARSER_DECLARATION, parser->tokens,
					parser->event_pw);
		}
		break;
	}
...
css_error parseInlineStyle(css_parser *parser)
{
	...
	case Initial:
		...
		if (parser->event != NULL) {
			...
			/* 2) begin ruleset */
			parser->event(parser->ctx, CSS_PARSER_START_RULESET, NULL,
					parser->event_pw);
		}
	...
	case AfterISBody0:
		...
		if (parser->event != NULL) {
			/* 1) end ruleset */
			parser->event(parser->ctx, CSS_PARSER_END_RULESET, NULL,
					parser->event_pw);
...
void discard_tokens(css_parser *parser)
{
	...
	while ((tok = parserutils_vector_iterate(
		...
		if (tok->idata != NULL) {
			lwc_string_unref(parser->ctx, tok->idata);
		}
	}
