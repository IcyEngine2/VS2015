 ...
 */
parserutils_error parserutils_inputstream_create(parserutils_realloc realloc, void* user, 
	const char *enc, uint32_t encsrc, parserutils_charset_detect_func csdetect,
		parserutils_inputstream **stream)
{
	parserutils_inputstream_private *s;
	parserutils_error error;

	if (stream == NULL || realloc == NULL)
		return PARSERUTILS_BADPARM;

	s = realloc(0, sizeof(parserutils_inputstream_private), user);
	if (s == NULL)
		return PARSERUTILS_NOMEM;

	error = parserutils_buffer_create(realloc, user, &s->raw);
	if (error != PARSERUTILS_OK) {
		realloc(s, 0, user);
		return error;
	}

	error = parserutils_buffer_create(realloc, user, &s->public.utf8);
	if (error != PARSERUTILS_OK) {
	...
	error = parserutils__filter_create(realloc, user, "UTF-8", &s->input);
	if (error != PARSERUTILS_OK) {
		...
		parserutils_inputstream *stream)
{
	...
	parserutils_buffer_destroy(s->public.utf8);
	parserutils_buffer_destroy(s->raw);
	s->public.realloc(s, 0, s->public.user);
