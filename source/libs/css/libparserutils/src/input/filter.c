 ...
 */
parserutils_error parserutils__filter_create(parserutils_realloc realloc, void* user, 
	const char *int_enc, parserutils_filter **filter)
{
	parserutils_filter *f;
	parserutils_error error;

	if (int_enc == NULL || filter == NULL || realloc == NULL)
		return PARSERUTILS_BADPARM;

	f = realloc(0, sizeof(parserutils_filter), user);
	if (f == NULL)
	...
	error = parserutils_charset_codec_create(realloc, user, int_enc, &f->write_codec);
	if (error != PARSERUTILS_OK) {
...
parserutils_error parserutils__filter_destroy(parserutils_filter *input)
{
	...
	input->realloc(input, 0, input->user);
		...
		const char *enc)
{
	...
	error = parserutils_charset_codec_create(input->realloc, input->user, enc, &input->read_codec);
	if (error != PARSERUTILS_OK)
		return error;
#endif
