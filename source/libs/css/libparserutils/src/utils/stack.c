 ...
 */
parserutils_error parserutils_stack_create(parserutils_realloc realloc, void* user,size_t item_size, size_t chunk_size,
		parserutils_stack **stack)
{
	parserutils_stack *s;

	if (item_size == 0 || chunk_size == 0 || stack == NULL || realloc == NULL)
		return PARSERUTILS_BADPARM;

	s = realloc(0, sizeof(parserutils_stack), user);
	if (s == NULL)
		return PARSERUTILS_NOMEM;

	s->items = realloc(0, item_size * chunk_size, user);
	if (s->items == NULL) {
		realloc(s, 0, user);
		return PARSERUTILS_NOMEM;
	}
...
parserutils_error parserutils_stack_destroy(parserutils_stack *stack)
{
	...
	stack->realloc(stack->items, 0, stack->user);
	stack->realloc(stack, 0, stack->user);
		...
		const void *item)
{
	...
	if ((size_t) slot >= stack->items_allocated) {
		void *temp = stack->realloc(stack->items,
				(stack->items_allocated + stack->chunk_size) *
				stack->item_size, stack->user);
		if (temp == NULL)
			return PARSERUTILS_NOMEM;
