 ...
 */
parserutils_error parserutils_vector_create(parserutils_realloc realloc, void* user, 
	size_t item_size, size_t chunk_size, parserutils_vector **vector)
{
	parserutils_vector *v;

	if (item_size == 0 || chunk_size == 0 || vector == NULL || realloc == NULL)
		return PARSERUTILS_BADPARM;

	v = realloc(0, sizeof(parserutils_vector), user);
	if (v == NULL)
		return PARSERUTILS_NOMEM;

	v->items = realloc(0, item_size * chunk_size, user);
	if (v->items == NULL) {
		realloc(v, 0, user);
		return PARSERUTILS_NOMEM;
	}
...
parserutils_error parserutils_vector_destroy(parserutils_vector *vector)
{
	...
	vector->realloc(vector->items, 0, vector->user);
	vector->realloc(vector, 0, vector->user);
		...
		void *item)
{
	...
	if ((size_t) slot >= vector->items_allocated) {
		void *temp = vector->realloc(vector->items,
				(vector->items_allocated + vector->chunk_size) *
				vector->item_size, vector->user);
		if (temp == NULL)
			return PARSERUTILS_NOMEM;
