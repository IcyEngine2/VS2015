 ...
 */
parserutils_error parserutils_buffer_create(parserutils_realloc realloc, void* user, parserutils_buffer **buffer)
{
	parserutils_buffer *b;

	if (buffer == NULL || realloc == NULL)
		return PARSERUTILS_BADPARM;

	b = realloc(0, sizeof(parserutils_buffer), user);
	if (b == NULL)
		return PARSERUTILS_NOMEM;

	b->data = realloc(0, DEFAULT_SIZE, user);
	if (b->data == NULL) {
		realloc(b, 0, user);
		return PARSERUTILS_NOMEM;
	}
...
parserutils_error parserutils_buffer_destroy(parserutils_buffer *buffer)
{
	...
	buffer->realloc(buffer->data, 0, buffer->user);
	buffer->realloc(buffer, 0, buffer->user);
...
{
	uint8_t *temp = buffer->realloc(buffer->data, buffer->allocated * 2, buffer->user);
	if (temp == NULL)
		return PARSERUTILS_NOMEM;
...
#ifndef NDEBUG
	temp = buffer->realloc(0, buffer->allocated, buffer->user);
	if (temp == NULL)
		return PARSERUTILS_NOMEM;
