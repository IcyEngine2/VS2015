...
#include <assert.h>
	...
	typedef void* (*lwc_realloc)(const void*, size_t, void*);
	...
	typedef enum lwc_error_e {
		...
		lwc_error_inval = 3,/**< Invalid parameter. */
	} lwc_error;

	extern lwc_context* lwc_initialise(lwc_realloc realloc, void* user);

	extern void lwc_shutdown(lwc_context* ctx);
	 ...
	 *	 on it.
	 */
	extern lwc_error lwc_intern_string(lwc_context* ctx, const char* s, size_t slen,
		lwc_string** ret);

	static inline lwc_error lwc_string_isequal(lwc_string* str1, lwc_string* str2, bool* ret)
	{
		*(ret) = ((str1) == (str2));
		return lwc_error_ok;
	}
	 ...
	 */
	extern lwc_error lwc_intern_substring(lwc_context* ctx, lwc_string* str,
	 ...
	 *             to by \a ret will not be valid.
	 */
	extern lwc_error lwc_string_tolower(lwc_context* ctx, lwc_string* str, lwc_string** ret);
	...
	extern void lwc_string_destroy(lwc_context* ctx, lwc_string* str);

	static inline void lwc_string_unref(lwc_context* ctx, lwc_string* str)
	{
		lwc_string* __lwc_s = (str);
		assert(__lwc_s != NULL);
		__lwc_s->refcnt--;
		if ((__lwc_s->refcnt == 0) ||
			...
			lwc_string_destroy(ctx, __lwc_s);
	}
	...
	extern lwc_error
	  ...
	  */
	static inline lwc_error lwc_string_caseless_isequal(lwc_context* ctx, lwc_string* _str1, lwc_string* _str2, bool* _ret)
	{
		...
		if (__lwc_str1->insensitive == NULL)
			__lwc_err = lwc__intern_caseless_string(ctx, __lwc_str1);
		if (__lwc_err == lwc_error_ok && __lwc_str2->insensitive == NULL)
			__lwc_err = lwc__intern_caseless_string(ctx, __lwc_str2);
	...
	static inline lwc_error lwc_string_caseless_hash_value(
		lwc_context* ctx, lwc_string* str, lwc_hash* hash)
	{
		assert(str != NULL);
		if (str->insensitive == NULL) {
			lwc_error err = lwc__intern_caseless_string(ctx, str);
			if (err != lwc_error_ok) {
				return err;
			}
		}
	 ...
	 */
	extern void lwc_iterate_strings(lwc_context* ctx, lwc_iteration_callback_fn cb, void* pw);
