...
{
public:
    display_system_data(const adapter adapter) : m_adapter(adapter)
    {

    }
    ~display_system_data() noexcept;
    error_type initialize(HWND__* const window) noexcept;
...
error_type display_system_data::initialize(HWND__* const window) noexcept
{
    ...
    ICY_COM_ERROR(device->CreateVertexShader(g_shader_bytes_screen_vs, sizeof(g_shader_bytes_screen_vs), nullptr, &m_vertex));
    context->VSSetShader(m_vertex, nullptr, 0);

    ICY_COM_ERROR(device->CreatePixelShader(g_shader_bytes_draw_texture_ps, sizeof(g_shader_bytes_draw_texture_ps), nullptr, &m_pixel));
    context->PSSetShader(m_pixel, nullptr, 0);

    ICY_COM_ERROR(device->CreateSamplerState(&CD3D11_SAMPLER_DESC(D3D11_DEFAULT), &m_sampler));
    context->PSSetSamplers(0, 1, &m_sampler);


    DXGI_SWAP_CHAIN_DESC desc;
    ICY_COM_ERROR(m_chain->GetDesc(&desc));
    ICY_ERROR(do_resize(window_size(desc.BufferDesc.Width, desc.BufferDesc.Height)));
...
error_type display_system_data::do_resize(const window_size size) noexcept
{
    ...
    ICY_COM_ERROR(device->CreateRenderTargetView(buffer, &CD3D11_RENDER_TARGET_VIEW_DESC(
...
error_type display_system_data::do_repaint(const bool vsync, const display_texture& texture) noexcept
{
    ...
    ID3D11ShaderResourceView* const srv[] = { texture.srv };
    ...
    while (true)
    {
        ...
        if (m_cpu_ready && m_gpu_ready)
        {
            ...
            if (m_frame.queue.pop(texture))
            {
                ...
                frame.time = clock_type::now() - now;
                frame.index = m_frame.index++;
                ICY_ERROR(event::post(this, event_type::display_update, std::move(frame)));
                reset();
...
error_type display_system_data::repaint(const texture& texture, const window_size offset, const window_size size) noexcept
{
    ...
    ICY_COM_ERROR(static_cast<ID3D11Texture2D*>(texture.handle())->QueryInterface(&dxgi));
    ...
    ICY_COM_ERROR(device->CreateShaderResourceView(new_texture.buffer,
        &CD3D11_SHADER_RESOURCE_VIEW_DESC(new_texture.buffer, D3D11_SRV_DIMENSION_TEXTURE2D, 
            DXGI_FORMAT_R8G8B8A8_UNORM, 0, 1), &new_texture.srv));
...
error_type icy::create_display_system(shared_ptr<display_system>& system, void* const handle, const adapter adapter) noexcept
{
    ...
    ICY_ERROR(ptr->initialize(static_cast<HWND__*>(handle)));
    system = std::move(ptr);
    return error_type();
}
